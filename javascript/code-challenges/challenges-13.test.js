...
// Solution code here...
if (arr.length > 1) {
  let longest = arr.reduce((arry1, arry2) => {
    return arry1.length > arry2.length ? arry1 : arry2;
  });
...
const findHappiness = (arr) => {
  // Solution code here...
  let filtered = arr.filter(item =>item.includes(":)"))
  return filtered;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.map((value) => {
    newArr.push(
      value.substring(1, 4) + value.substring(6, 9) + value.substring(10, 14)
    );
  });
  return newArr;
};
